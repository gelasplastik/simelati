<x-layouts.app :title="'Jadwal Mengajar'" :pageTitle="'Jadwal Mengajar'" :breadcrumb="'Admin / Master Data / Jadwal Mengajar'">
    ...
    <x-panel title="Tambah Jadwal" class="mb-3">
        <form method="POST" action="{{ route('admin.teaching-schedules.store') }}" class="row g-2">
            @csrf
            <div class="col-md-3">
            ...
            <div class="col-md-1"><input type="number" class="form-control" name="jam_ke" min="1" max="12" placeholder="Jam" required></div>
            <div class="col-md-1"><input type="time" class="form-control" name="start_time"></div>
            <div class="col-md-1"><input type="time" class="form-control" name="end_time"></div>
    ...
    <x-panel>
        <table class="table table-sm align-middle">
            <thead><tr><th>Profil</th><th>Guru</th><th>Kelas</th><th>Mapel</th><th>Hari</th><th>Jam Ke</th><th>Waktu</th><th>Status</th><th>Aksi</th></tr></thead>
            <tbody>
            @forelse($items as $item)
                <tr>
                    ...
                    <td>
                        <form method="POST" action="{{ route('admin.teaching-schedules.update', $item) }}" class="row g-1">
                            @csrf
                            @method('PUT')
                            <div class="col-md-2">
                            ...
                            <div class="col-md-1"><input type="number" class="form-control form-control-sm" name="jam_ke" min="1" max="12" value="{{ $item->jam_ke }}" required></div>
                            <div class="col-md-1"><input type="time" class="form-control form-control-sm" name="start_time" value="{{ $item->start_time }}"></div>
                            <div class="col-md-1"><input type="time" class="form-control form-control-sm" name="end_time" value="{{ $item->end_time }}"></div>
                            <div class="col-md-1 d-flex align-items-center">
                                <div class="form-check"><input type="checkbox" class="form-check-input" name="is_active" value="1" @checked($item->is_active)></div>
                            </div>
                            <div class="col-md-1"><button class="btn btn-sm btn-outline-primary w-100">Update</button></div>
                        </form>
                    </td>
                </tr>
            @empty
