<x-layouts.app :title="'Rekap Kehadiran Siswa'" :pageTitle="'Rekap Kehadiran Siswa'" :breadcrumb="'Admin / Reports / Rekap Kehadiran Siswa'">
    <x-panel class="mb-3">
        <form method="GET" action="{{ route('admin.reports.student-attendance-recap') }}" class="row g-2">
            <div class="col-md-4">
            ...
            <div class="col-md-2">
                @if($recap)
                    <a class="btn btn-outline-success w-100" href="{{ route('admin.reports.student-attendance-recap.export', ['class_id' => $selectedClassId, 'month' => $month, 'year' => $year]) }}">CSV</a>
                @endif
            </div>
        </form>
    </x-panel>
